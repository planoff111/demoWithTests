...
public class ControllerTests {
    ...
    @Test
    @DisplayName("GET API -> /api/users/ukrainians")
    @WithMockUser(roles = "ADMIN")
    public void getAllByUkraine() throws Exception {
        var employee = Employee.builder().id(1).name("Ivan").country("Ukraine").build();
        var employeeTwo = Employee.builder().id(2).name("John").country("UK").build();
        var employeeThree = Employee.builder().id(3).name("Eugene").country("US").build();

        List<Employee> list = Arrays.asList(employee, employeeTwo, employeeThree);

        List<EmployeeDto> employeeDtos = new ArrayList<>();

        when(service.getAllUrainians()).thenReturn(list);
        when(employeeMapper.toListEmployeeDto(list)).thenReturn(employeeDtos);

        MockHttpServletRequestBuilder mockRequest = get("/api/users/ukrainians")
        ...
        verify(service).getAllUrainians();
    }
